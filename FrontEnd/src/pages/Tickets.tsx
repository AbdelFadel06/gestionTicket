        ...
        <TableBody>
          {tickets.map((ticket, i) => (
            <TableRow key={ticket.id}>
              ...
              <TableCell>{ticket.developer?.username ?? 'Non assigné'}</TableCell>
              <TableCell>
                <div className="flex gap-2 justify-end">
                  {user?.role?.toLowerCase() === 'developer' && !ticket.developer && (
                    <Button size="sm" onClick={() => handleAssignToMe(ticket.id)}>
                      Prendre en charge
                    </Button>
                  )}
                  {ticket.author?.id === user?.id && (
                    <Button size="sm" variant="destructive" onClick={() => deleteTicket(ticket.id)}>
                      Supprimer
                    </Button>
                  )}
                  <Button
          ...
          {selectedTicket ? (
            <>
              <DialogHeader>
                <DialogTitle>{selectedTicket.title}</DialogTitle>
                <DialogDescription>
                  Priorité : {selectedTicket.priority ?? '—'} | Status : {selectedTicket.status}
                </DialogDescription>
              </DialogHeader>
